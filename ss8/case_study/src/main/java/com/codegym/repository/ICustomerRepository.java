...
public interface ICustomerRepository extends JpaRepository<Customer, Integer> {
    @Query(value = "select c.* from `customer` c  join `customer_type` ct  on c.customer_type_id = ct.id " +
            ...
            "and customer_type_id = :customerType and c.status = false", nativeQuery = true)
    ...
    @Query(value = "select * from customer where name like concat('%', :name, '%') and status = false ",nativeQuery = true)
    Page<Customer> listCustomer(@Param("name") String name, Pageable pageable);
}
